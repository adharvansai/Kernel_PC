...
class Kernel_Layer(nn.Module):
    ...
    def _k_means(self, X):
        ...
        kmeans_clusters = KMeans(n_clusters=self.hidden_dim).fit(X)
    ...
    def forward(self, x):
        ...
        rbf_shape = (x.shape[0],self.prototypes.shape[0],x.shape[1])
        x_i = x.unsqueeze(1).expand(rbf_shape)
        x_p = self.prototypes.unsqueeze(0).expand(rbf_shape)
        norm2 = np.linalg.norm((x_i-x_p),2,axis=2)
        k_meth = torch.tensor(np.exp(-1*norm2/(2*(self.sigma**2))))
        return k_meth
...
class Kernel_LR(nn.Module):
    ...
    def __init__(self, sigma, hidden_dim):
        ...
        kern_obj = Kernel_Layer(sigma)
        self.net = nn.Sequential(kern_obj,nn.Linear(self.hidden_dim,1,bias=False))
...
class RBF(nn.Module):
    ...
    def __init__(self, sigma, hidden_dim):
        ...
        # kernel layer (Kernel_Layer object) and a linear layer (nn.Linear object)
        # How should we set the "bias" argument of nn.Linear?
        kern_obj = Kernel_Layer(sigma,hidden_dim)
        self.net = nn.Sequential(kern_obj, nn.Linear(hidden_dim, 1, bias=False))
...
class FFN(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim):
        ...
        self.net = nn.Sequential(nn.Linear(input_dim,hidden_dim,bias=False), nn.Linear(hidden_dim, 1, bias=False))
        ### END CODE HERE
